<?php 
	...
	if ($_GET) {
		if (isset($_GET['id']) && !empty($_GET['id'])) {
			...
			if ($newsletter_id) {
				$act = substr(md5("Delete-Message-".$newsletter_id.$_SESSION['token']), 3,15);
					if($act == $_GET['act']){
					...
					if ($newsletter_info) {
						...
						if ($success) {
							redirect('../newsletter','success','Newsletter deleted Successfully');
						}else{
							redirect('../newsletter','error','Error while deleting Newsletter');
						}
					}else{
						redirect('../newsletter','error','Newsletter not Found');
					}
				}else{
